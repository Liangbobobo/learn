...
# 2.7IPv6寻址
IPv6地址有128位长，一个IPv6可以看作是一个字符串，它唯一标识了全球Internet中的一个网卡，这个128位字符又可以理解为包含有网络部分和主机部分的地址。   
每个IPv6有128位，有8个字段，每个字段16位，一个十六进制的字符是4位，所以每个字段有4个十六进制的字符。

不管是IPv4还是IPv6这两个版本的地址，实际上都是二进制数，它们都是由0和1组成的字符串。

## 2.7.4接口标识符
IPv6要求，每个网卡具有其唯一的标识符，即接入IPv6的每个设备中的每个网卡都必须有自己唯一的接口标识符（interface indentifier），但是在特殊情况下，某些具有多个网卡的主机，可能要在所有这些网卡之间实现动态载荷平衡，而这些网卡可能是共享一个标识符。

## 2.7.6 IPv6的地址类型
1. 特殊地址  
有两个地址保留作为特殊用途：非特定地址和回送地址。  
unspecified address非特定地址：全为0的地址，可以用两个冒号：：来表示，主要用于那些机器启动时还不知道自己的地址但是又必须发送消息的节点。如，这种节点可能需要给本地链路中线连接的路由器发送一个消息，以通告其地址，这样新节点就知道其位置了。   
loopback address回送地址：是一种特殊的IP地址名，它允许网络中的主机检查对其自己的本地TCP/IP协议栈的操作。回送地址除了最后一位是1，其他位全为0，可表示位::1。回送地址是一种只能在本地使用的诊断工具，不能用作数据包真正发往网络时的源地址或目标地址。当数据包将回送地址作为其目标地址进行发送时，发送主机上的IPv6栈只是把消息发送给自己，即沿栈从上往下，再从下往上返回，甚至都不会访问实际的网卡。这种操作要求确保设备的IP栈已正确安装和配置好（如果一个设备不能与自己对话，肯定也不能与其他设备对话）

众所周知，localhost的IPv4地址是127.0.0.1(回送地址)，实际上，127.0.0.0/8中的任何IPv4地址都是回送地址。

2. 多播地址multicast address  
用于给多个主机发送相同的消息。  
在本地以太网中，主机可以侦听它们预定的多播数据。  
在其他网络中，多播数据必须以一种不同的方式来处理，有时需要一台专门的服务器，把多播数据转发给每个预定主机。